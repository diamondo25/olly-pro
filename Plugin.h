...
// that character is set to unsigned.
  ...
  // Add some more for exported functions
...
// Switch between exported and non-exported functions
#ifdef _OLLYPLUGIN
...
#define _export        __declspec(dllexport)
    ...
    #define _export        __declspec(dllexport)
    #ifdef __cplusplus
      #define extc           extern "C" _export    // Assure that names are not mangled
    #else
...
// Prototypes for plugin callback functions.
typedef int  cdecl (*ODBG_Plugindata)(char shortname[32]);
typedef int  cdecl (*ODBG_Plugininit)(int ollydbgversion,HWND hw,
                     ulong *features);
typedef void cdecl (*ODBG_Pluginmainloop)(DEBUG_EVENT *debugevent);
typedef void cdecl (*ODBG_Pluginsaveudd)(t_module *pmod,int ismainmodule);
typedef int  cdecl (*ODBG_Pluginuddrecord)(t_module *pmod,int ismainmodule,
                     ulong tag,ulong size,void *data);
typedef int  cdecl (*ODBG_Pluginmenu)(int origin,char data[4096],void *item);
typedef void cdecl (*ODBG_Pluginaction)(int origin,int action,void *item);
typedef int  cdecl (*ODBG_Pluginshortcut)(
                    int origin,int ctrl,int alt,int shift,int key,
                    void *item);
typedef void cdecl (*ODBG_Pluginreset)(void);
typedef int  cdecl (*ODBG_Pluginclose)(void);
typedef void cdecl (*ODBG_Plugindestroy)(void);
typedef int  cdecl (*ODBG_Paused)(int reason,t_reg *reg);
typedef int  cdecl (*ODBG_Pausedex)(int reasonex,int dummy,t_reg *reg,
                    DEBUG_EVENT *debugevent);
typedef int  cdecl (*ODBG_Plugincmd)(int reason,t_reg *reg,char *cmd);
#endif